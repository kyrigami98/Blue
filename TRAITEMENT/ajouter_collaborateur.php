<?php
    ...
    {
        $requete = $bdd->prepare('SELECT email_utilisateur FROM collaborer, (SELECT id_utilisateur, email_utilisateur FROM utilisateur)collaborateur WHERE collaborer.id_utilisateur = collaborateur.id_utilisateur AND email_utilisateur = :email');

        $requete->execute(array('email' => $_POST['collaborateur']));

        $donnee = $requete->fetch();

        if($donnee == false)
        {
            $requete1 = $bdd->prepare('SELECT id_utilisateur, type_utilisateur FROM utilisateur WHERE email_utilisateur = :email');
